...
void selectionSort(vector<Bid>& bids) {
    ...
    for (size_t pos = 0; pos < largest - 1; ++pos) {  // Loop until second-to-last element
        size_t min = pos;  // Assume that the current position is the minimum

        // Loop over remaining elements to the right of pos
        for (size_t j = pos + 1; j < largest; ++j) {
            // If this element's title is lexicographically smaller than the current minimum's title
            if (bids[j].title.compare(bids[min].title) < 0) {
                min = j;  // Update min to the new minimum element's index
            }
        }

        // Swap the current minimum with the element at position pos, if it's not already in the correct place
        if (min != pos) {
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 3: // selectionSort is utilized to sort loaded bids and elapsed time is determined and displayed
            ticks = clock();  // Start clock before sorting

            // Invoke selectionSort to sort the bids
            selectionSort(bids);

            // Calculate elapsed time after sorting
            ticks = clock() - ticks;  // Subtract start time from current clock ticks

            std::cout << bids.size() << " bids read" << std::endl;

            // Display the time in clock ticks
            std::cout << "time: " << ticks << " clock ticks" << std::endl;

            // Display the time in seconds (clock ticks / CLOCKS_PER_SEC)
